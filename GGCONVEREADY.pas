...
uses
  ...
  Dialogs, GGELABORA, Grids, dbgrids, RzDBGrid, ADODB, DB, MyAccess, query_go,
...
begin
        ...
        tabella_02.fieldbyname('descrizione2').asstring :=
          trim(copy(tabella_clifor.fieldbyname('des_ragsoc').asstring, j + 1, length(tabella_clifor.fieldbyname('des_ragsoc').asstring) - j));
        ...
        *)
        ...
        begin
          ...
          tabella_01.fieldbyname('iban').asstring := copy(stringa, 1, 4) + ' ' + copy(stringa, 5, 4) + ' ' +
            copy(stringa, 9, 4) + ' ' + copy(stringa, 13, 4) + ' ' + copy(stringa, 17, 4) + ' ' +
            copy(stringa, 21, 4) + ' ' + copy(stringa, 25, 3);
...
begin
      ...
      begin
        ...
        begin
          ...
          tabella_02.fieldbyname('descrizione2').asstring :=
            trim(copy(tabella_esa_03.fieldbyname('des_ragsoc').asstring, j + 1, length(tabella_esa_03.fieldbyname('des_ragsoc').asstring) - j));
...
begin
  ...
  begin
    ...
    begin
      ...
      begin
        if (tabella_02.fieldbyname('codice_01').asstring = '03') or
          (tabella_02.fieldbyname('codice_01').asstring = '04') then
        begin
...
begin
  ...
  begin
    ...
    if not tabella_01.locate('art_codice;codice_barre', Vararrayof([
      trim(tabella_esa_02.fieldbyname('cod_art').asstring),
      trim(tabella_esa_02.fieldbyname('cod_agg').asstring)]), []) then
    begin
      tabella_01.append;
...
begin
  ...
  begin
    ...
    begin
      ...
      if (tabella_esa_02.fieldbyname('cod_caumag_r').asstring <> '000') and
        (tabella_esa_02.fieldbyname('cod_caumag_r').asstring <> '') then
  ...
  begin
    ...
    begin
      ...
      while
        ...
        (articolo = trim(tabella_esa_01.fieldbyname('cod_art').asstring)) and
        (listino = trim(tabella_esa_01.fieldbyname('cod_listino').asstring)) and
        (prg_listino = tabella_esa_01.fieldbyname('prg_listino').asinteger) do
      begin

        if read_tabella(arc.arcdit, 'art', 'codice', tabella_esa_01.fieldbyname('cod_art').asstring) then
        begin
          if not tabella_01.locate('art_codice;' + campo_listino,
            VarArrayOf([
            trim(tabella_esa_01.fieldbyname('cod_art').asstring),
            trim(tabella_esa_01.fieldbyname('cod_listino').asstring)])
            , []) then
          begin
            tabella_01.append;
            ...
            begin
              ...
              begin
                crea_tsm(
                  tabella_esa_01.fieldbyname('prc_sconto1').asfloat,
                  tabella_esa_01.fieldbyname('prc_sconto2').asfloat);
...
begin
  ...
  if (tabella_esa_01.fieldbyname('cod_attivita').asstring = '01') and
    (tabella_esa_01.fieldbyname('num_reg_iva').asstring = '2') then
  begin
    cod_caucon := cod_caucon + tabella_esa_01.fieldbyname('num_reg_iva').asstring;
  end;

  if (tabella_esa_01.fieldbyname('cod_attivita').asstring = '02') and
    (tabella_esa_01.fieldbyname('sig_serie_doc').asstring = '') then
  begin
    cod_caucon := cod_caucon + tabella_esa_01.fieldbyname('cod_attivita').asstring;
  end;

  if (tabella_esa_01.fieldbyname('cod_attivita').asstring = '02') and
    (tabella_esa_01.fieldbyname('sig_serie_doc').asstring = 'BIS') then
  begin
...
begin
  ...
  begin
    ...
    if (test_data <> tabella_esa_01.fieldbyname('dat_movimento').asdatetime) or
      (test_numero_stringa <> tabella_esa_01.fieldbyname('des_num_doc').asstring) or
      (test_cod_causale <> trim(tabella_esa_01.fieldbyname('cod_caumag').asstring)) then
    begin
      progressivo := progressivo + 1;
...
begin
  ...
  while not tabella_esa_03.eof do
  begin
    if (tabella_esa_03.fieldbyname('cod_tipoomag').asstring = '') or
      ((tabella_esa_03.fieldbyname('cod_tipoomag').asstring <> '') and (tabella_esa_03.fieldbyname('val_tot_imp').asfloat > 0)) then
    begin
      try
        tabella_03.append;
...
begin
  ...
  begin
    ...
    if (codice_articolo <> '-M') and
      (codice_articolo <> '-D') and
      (codice_articolo <> '') then
    begin
      esiste_articolo := true;
...
begin
  query.sql.clear;
  query.sql.Add('truncate table ' + tabella);
  query.execsql;
...
registerclass(tCONVEREADY);
  ...
  t.*
  FROM MG_MOVMAGT t
  left join[AZI_ENI].[dbo].CO_DOCUMENTI co on co.num_esercizio = t.num_esercizio and co.dat_doc = t.dat_doc and co.des_num_doc = t.des_num_doc
  left join[AZI_ENI].[dbo].CO_DOCUMENTI_PN cp on cp.prg_documento = co.prg_documento
  where
  t.cod_gruppo_tipodoc = 'FAV'
